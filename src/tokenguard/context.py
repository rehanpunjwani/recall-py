...
from tokenguard.retrieve import build_ann_index, top_k_chunks
from tokenguard.router import rough_token_count
...
def format_context_injection(
    ...
    *,
    ...
    if not citations:
        return (
            "(No indexed context yet. Run `tokenguard index` in the repo root, "
...
def build_agent_context_message(
    *,
    ...
    lines = [
        "TokenGuard — use this retrieved context before answering.",
    ...
    if top_score >= 0.7:
        ...
        lines.append(
            ...
            "Call MCP `tokenguard_escalate_pack` for complex multi-step work."
        )
    else:
        lines.append(
            "Policy: Weak context match — read project files. "
            "Run `tokenguard index` if docs are missing. "
            "Use `tokenguard_escalate_pack` before heavy reasoning."
        )
    lines.append(
        "After your final reply, call MCP `tokenguard_ingest_turn` with role=assistant "
